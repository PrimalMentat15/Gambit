...
implemented twice. Nothing in this module imports Qt, so the CLI works with no
...
def run_pids(meta: Dict) -> Dict[str, Optional[int]]:
    ...
    """
    return {
        "trainer": meta.get("trainer_pid"),
        "balatro": meta.get("balatro_pid"),
    }
...
def describe(meta: Dict) -> List[str]:
    ...
    for role, pid in run_pids(meta).items():
        if not pid:
            ...
            continue
        lines.append(f"  {role:<9} pid {pid}  {'running' if is_alive(pid) else 'not running'}")
    return lines
