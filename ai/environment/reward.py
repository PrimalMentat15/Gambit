...
from ..telemetry import EventType, emit

# Per-episode win breakdowns are noisy at 2 steps/sec over a 33h run; they are
# always emitted as telemetry, and printed only when this is set.
VERBOSE = os.environ.get("BALATRO_RL_VERBOSE_REWARD", "1") not in ("0", "false", "False")
...
class BalatroRewardCalculator:
    ...
    def __init__(self):
        ...
        # Reward components that fired on the most recent step. Read by the
        # environment for the info dict and telemetry.
        self.last_breakdown: List[str] = []
    ...
    def calculate_reward(self, current_state: Dict[str, Any], 
        ...
        # Expose which components fired so the caller can log or emit them
        self.last_breakdown = reward_breakdown
    ...
    def _log_episode_win(self):
        """Emit and optionally print a breakdown of a winning episode"""
        ...
        emit(
            ...
            hands_used=len(self.hands_played),
            total_reward=round(self.episode_total_reward, 3),
            hands_played=list(self.hands_played),
        )
    ...
    def _log_win_rate(self):
        """Emit and optionally print win rate statistics"""
