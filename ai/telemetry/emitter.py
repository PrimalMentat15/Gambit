...
The training loop runs at roughly 2 steps/sec and every step blocks on the game,
so the one rule that matters is that ``emit()`` must never block or raise. It
does a dict construction and a non-blocking queue put; if the queue is full the
event is dropped and counted. A dead or slow consumer can therefore never slow
down or crash training.
...
def set_emitter(emitter: Any) -> None:
    """Install the process-wide emitter"""
    global _active
    _active = emitter
