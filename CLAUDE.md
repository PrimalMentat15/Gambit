...
The mod talks to the AI over a single localhost TCP connection:
- **Address**: `127.0.0.1:12345`, overridable via `BALATRO_RL_HOST` / `BALATRO_RL_PORT` (read by both sides)
- **Roles**: Python (`ai/utils/communication.py`) is the server; the mod (`RLBridge/communication.lua`) is the client
- **Protocol**: Line-delimited JSON, one message per line, `TCP_NODELAY` enabled
- **Flow**:
  1. Python binds and blocks on `accept()`
  2. Balatro connects lazily on the first action request
  3. Balatro writes a JSON request line
  4. AI reads the request, writes a JSON response line
  5. Balatro performs a blocking read for the response

### Benefits of a Socket
- Identical code path on Linux, Windows and macOS (no `mkfifo`)
- No extra dependencies: Python stdlib `socket`, and LuaSocket is compiled into LÖVE's `love.dll`
- Blocking reads ensure proper synchronization
- Connection drops are detected and re-established automatically on game relaunch
...
- Communicate with AI via localhost socket
